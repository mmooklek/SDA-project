...
# (2.) Histrograms
# Canton of Zurich
fig_zh, axes_zh = plt.subplots(2, 3, figsize=(15, 10))  
fig_zh.patch.set_alpha(0.0)

for i, var in enumerate(continuous_vars_zh):
    ax = plt.subplot(3, 3, i+1)  # Adjust the grid size according to the number of variables
    plt.hist(data_zh[var].dropna(), bins=20, edgecolor='black')
    plt.title(f'Histogram of {var}')
    plt.xlabel(var)
    plt.ylabel('Frequency')
    ax.set_facecolor('none')
    fig_zh.patch.set_facecolor('none') 

plt.tight_layout()
plt.show()

# City of Zurich
fig_zh, axes_zh = plt.subplots(3, 3, figsize=(15, 10))  
fig_zh.patch.set_alpha(0.0)
...
sm.qqplot(data_zh['Rent'], line ='45', fit=True, ax=ax_zh)
ax_zh.set_title("QQ Plot for Canton of Zurich")
ax_zh.set_xlabel("Theoretical Quantiles")
ax_zh.set_ylabel("Sample Quantiles")
ax_zh.lines[1].set_color("black")
ax_zh.get_lines()[0].set_color("steelblue")
ax_zh.get_lines()[0].set_markerfacecolor("steelblue")
ax_zh.get_lines()[0].set_markeredgecolor("steelblue")
ax_zh.set_facecolor('none')  # Set axes background to transparent
ax_zh.set_aspect('equal', adjustable='box')  # Make the plot box square
fig_zh.patch.set_facecolor('none')  # Set figure background to transparent
plt.show()

# City of Zurich
fig_czh, ax_czh = plt.subplots()
sm.qqplot(data_czh['Rent'], line ='45', fit=True, ax=ax_czh)
ax_czh.set_title("QQ Plot for City of Zurich")
ax_czh.set_xlabel("Theoretical Quantiles")
ax_czh.set_ylabel("Sample Quantiles")
ax_czh.lines[1].set_color("black")
ax_czh.get_lines()[0].set_color("steelblue")
ax_czh.get_lines()[0].set_markerfacecolor("steelblue")
ax_czh.get_lines()[0].set_markeredgecolor("steelblue")
ax_czh.set_facecolor('none')
ax_czh.set_aspect('equal', adjustable='box')
fig_czh.patch.set_facecolor('none')
plt.show()
...
# Canton of Zurich Boxplot
fig_zh, ax_zh = plt.subplots(2, 3, figsize=(15, 10))  # Adjust the grid dimensions accordingly
fig_zh.patch.set_alpha(0.0)  # Set the figure background to transparent

# Loop through the variables and create a boxplot for each
for i, var in enumerate(continuous_vars_zh):
    ax = ax_zh.flat[i]
    sns.boxplot(y=data_zh[var], ax=ax)
...
# City of Zurich Boxplot
fig_czh, ax_czh = plt.subplots(3, 3, figsize=(15, 15))  # Adjust the grid dimensions accordingly
fig_czh.patch.set_alpha(0.0)  # Set the figure background to transparent

# Loop through the variables and create a boxplot for each
for i, var in enumerate(continuous_vars_czh):
    ax = ax_czh.flat[i]
    sns.boxplot(y=data_czh[var], ax=ax)
    ax.set_title(f'Boxplot of {var}')
    ax.set_xlabel(var)
    ax.set_facecolor('none')  # Set axes background to transparent
...
fig_zh, ax_zh = plt.subplots(figsize=(10, 7))
average_rent_sorted_zh.plot(kind='bar', color='steelblue', ax=ax_zh)
ax_zh.set_facecolor('none')
fig_zh.patch.set_facecolor('none')
...
# Create and show the bar chart
fig_czh, ax_czh = plt.subplots(figsize=(10, 7))
average_rent_sorted_czh.plot(kind='bar', color='steelblue', ax=ax_czh)
ax_czh.set_facecolor('none')
fig_czh.patch.set_facecolor('none')
plt.title('Average Rental Price by Neighborhood in the City of Zurich')
plt.xlabel('Neighborhood')
plt.ylabel('Average Rental Price')
plt.xticks(rotation=45)
plt.tight_layout()
...
# Create and show the bar chart
fig_zht, ax_zht = plt.subplots(figsize=(10, 7))
average_rent_sorted_zht.plot(kind='bar', color='steelblue', ax=ax_zht)
ax_zht.set_facecolor('none')
fig_zht.patch.set_facecolor('none')
...
# Create and show the bar chart
fig_czht, ax_czht = plt.subplots(figsize=(10, 7))
average_rent_sorted_czht.plot(kind='bar', color='steelblue', ax=ax_czht)
ax_czht.set_facecolor('none')
fig_czht.patch.set_facecolor('none')
...
# Create and show the bar chart
fig_zht1, ax_zht1 = plt.subplots(figsize=(10, 7))
average_rent_sorted_zht.plot(kind='bar', color='steelblue', ax=ax_zht1)
ax_zht1.set_facecolor('none')
fig_zht1.patch.set_facecolor('none')
...
# Create and show the bar chart
fig_zht2, ax_zht2 = plt.subplots(figsize=(10, 7))
average_rent_sorted_czht.plot(kind='bar', color='steelblue', ax=ax_zht2)
ax_zht2.set_facecolor('none')
fig_zht2.patch.set_facecolor('none')
...
fig.patch.set_alpha(0.0)  # Set the transparency for the figure background

# Plot the horizontal bar chart
bars = ax.barh(
    [name for name, _ in reversed(top_15_importance_tree)],
    [importance for _, importance in reversed(top_15_importance_tree)]
)

# Add the horizontal grid and remove the vertical grid
ax.xaxis.grid(True)
ax.yaxis.grid(False)
...
for bar in bars:
    width = bar.get_width()
    label_x_pos = width + max(feature_importances_dt) * 0.005
...
# Add vertical line at the maximum importance score
max_importance = max(feature_importances_dt)
ax.axvline(max_importance, color='grey', linestyle='--', linewidth=1)
...
# Apply tight layout to adjust for the plot elements
plt.tight_layout()
...
# Create the figure and axis for Random Forest
fig_rf, ax_rf = plt.subplots(figsize=(20, 10))
fig_rf.patch.set_alpha(0.0)  # Set the figure background to transparent

# Plot the horizontal bar chart for Random Forest
bars_rf = ax_rf.barh(
    [name for name, _ in reversed(top_15_importance_rf)],
    [importance for _, importance in reversed(top_15_importance_rf)]
)

# Add horizontal grid lines and remove the vertical ones
ax_rf.xaxis.grid(True, linestyle='--', alpha=0.7)
ax_rf.yaxis.grid(False)

# Annotate each bar with the importance score
for bar in bars_rf:
    width = bar.get_width()
    label_x_pos = width + max(feature_importances_rf) * 0.005
    ax_rf.text(label_x_pos, bar.get_y() + bar.get_height() / 2, f'{width:.3f}', va='center')

# Set the background color of the axes to be transparent
ax_rf.set_facecolor('none')

# Add vertical line at the maximum importance score for Random Forest
max_importance_rf = max(feature_importances_rf)
ax_rf.axvline(max_importance_rf, color='grey', linestyle='--', linewidth=1)

# Set the title and labels for Random Forest
ax_rf.set_title('Top 15 Random Forest Feature Importances for Canton of Zurich')
...
fig_lasso, ax_lasso = plt.subplots(figsize=(20, 10))
fig_lasso.patch.set_alpha(0.0)  # Set the figure background to transparent

# Plot the horizontal bar chart for Lasso
bars_lasso = ax_lasso.barh(y=top_15_importance_lasso.index[::-1], width=top_15_importance_lasso.values[::-1])

# Add horizontal grid lines and remove the vertical ones
ax_lasso.xaxis.grid(True, linestyle='--', alpha=0.7)
ax_lasso.yaxis.grid(False)

# Annotate each bar with the absolute coefficient value
for bar in bars_lasso:
    width = bar.get_width()
    label_x_pos = width + max(importance_lasso) * 0.005
    ax_lasso.text(label_x_pos, bar.get_y() + bar.get_height() / 2, f'{width:.3f}', va='center')

# Set the background color of the axes to be transparent
ax_lasso.set_facecolor('none')

# Add vertical line at the maximum importance score for Lasso
max_importance_lasso = max(importance_lasso)
ax_lasso.axvline(max_importance_lasso, color='grey', linestyle='--', linewidth=1)

# Set the title and labels for Lasso
ax_lasso.set_title('Top 15 Lasso Regression Feature Importances for Canton of Zurich')
ax_lasso.set_xlabel('Absolute Coefficient Value')
ax_lasso.set_ylabel('Features')

# Apply tight layout to adjust for the plot elements
plt.tight_layout()

# Show the plot for Lasso
plt.show()
...
# Plot the horizontal bar chart
bars = ax.barh(
    [name for name, _ in reversed(top_15_importance_tree_czh)],
...
for bar in bars:
    width = bar.get_width()
    label_x_pos = width + max(feature_importances_dt_czh) * 0.005
    ax.text(label_x_pos, bar.get_y() + bar.get_height() / 2, f'{width:.3f}', va='center')
...
# Plot the horizontal bar chart for Random Forest
bars_rf = ax_rf.barh(
    [name for name, _ in reversed(top_15_importance_rf_czh)],
...
# Add vertical line at the maximum importance score for Random Forest
max_importance_rf_czh = max(feature_importances_rf)
ax_rf.axvline(max_importance_rf_czh, color='grey', linestyle='--', linewidth=1)

# Set the title and labels for Random Forest
ax_rf.set_title('Top 15 Random Forest Feature Importances for City of Zurich')
ax_rf.set_xlabel('Importance')
ax_rf.set_ylabel('Features')
...
# Plot the horizontal bar chart for Lasso
bars_lasso = ax_lasso.barh(y=top_15_importance_lasso_czh.index[::-1], width=top_15_importance_lasso_czh.values[::-1])
...
for bar in bars_lasso:
    width = bar.get_width()
    label_x_pos = width + max(importance_lasso_czh) * 0.005
    ax_lasso.text(label_x_pos, bar.get_y() + bar.get_height() / 2, f'{width:.3f}', va='center')
...
# Add vertical line at the maximum importance score for Lasso
max_importance_lasso_czh = max(importance_lasso_czh)
ax_lasso.axvline(max_importance_lasso_czh, color='grey', linestyle='--', linewidth=1)

# Set the title and labels for Lasso
ax_lasso.set_title('Top 15 Lasso Regression Feature Importances for City of Zurich')
ax_lasso.set_xlabel('Absolute Coefficient Value')
ax_lasso.set_ylabel('Features')
